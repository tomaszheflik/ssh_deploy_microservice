...
func main() {
	sv := Version{"0.1.0"} // Define version of microservice
	r := mux.NewRouter()
	r.HandleFunc("/", sv.handler).Methods("GET")
	r.HandleFunc("/deploy/", sv.handler).Methods("GET")
	r.HandleFunc("/deploy/json/", deployJson).Methods("POST")
	r.HandleFunc("/ssh/", sv.handler).Methods("GET")
